    ...
    def run(self, df: pd.DataFrame, mode: str = "synthetic", regime: str = "HEAVY_NOISE") -> Dict[str, Any]:
        ...
        for iteration in range(1, self.MAX_ITERATIONS + 1):
            ...
            try:
                ...
                print(f"\n  [PIPELINE] [ERROR] TickEngine crashed: {e}")
                break
            ...
            try:
                ...
                print(f"\n  [PIPELINE] [APPROVED] APPROVED on iteration {iteration}. Filter: [{filter_name}]")
                break   # Exit the ReAct loop — we have a winner

            except RiskVetoException as e:
                risk_ms = round((time.time() - t0) * 1000)
                print(f"\n  [PIPELINE] [WARN] Iteration {iteration} VETOED by Risk Officer.")
        ...
        if approved_payload is None:
            quarantined = True
            print(f"\n{'='*68}")
            print(f"  [WARN] ALL {self.MAX_ITERATIONS} ITERATIONS VETOED - ENTERING QUARANTINE")
        ...
        export_df = final_filtered_df.copy()
        export_df['Raw_Bid'] = df['Bid'].values
        if 'Ask' in df.columns:
            export_df['Raw_Ask'] = df['Ask'].values
        ...
        print(f"  MQL5 by Gemini:  {'[OK] Yes' if gemini_used_mql5 else '[WARN] Fallback template'}")
        ...
        for k, v in viz_result.get("chart_paths", {}).items():
            print(f"    [CHART] {k}: {v}")
        print(f"    [MQL5] MQL5: {mql5_result.get('filepath', 'N/A')}")
...
if __name__ == "__main__":
    ...
    if args.mode == "real":
        if not DATA_LOADER_AVAILABLE:
            print("  [ERROR] data_loader.py not found. Cannot use --mode real.")
            sys.exit(1)
        ...
        if df.empty:
            print("  [ERROR] No data returned. Check dates or network connection.")
            sys.exit(1)
        print(f"  [SUCCESS] Loaded {len(df):,} real ticks.")
    else:
        ...
        print(f"  [SUCCESS] Synthetic data ready: {len(df):,} rows")
